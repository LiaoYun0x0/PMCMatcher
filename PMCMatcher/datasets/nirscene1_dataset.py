...
class nirscene1Dataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        opt_img_path = os.path.join(os.path.dirname(self.data_file), 'country', opt)
        ...
        sar_img_path = os.path.join(os.path.dirname(self.data_file), 'country', sar)
        ...
        #sar_img = cv2.resize(sar_img, self.size)
...
def build_nir(
        ...
        size,
        stride):
    train_data = nirscene1Dataset(
        train_data_file,
        size=(320, 320),
        stride=8)
    test_data = nirscene1Dataset(
        test_data_file,
        size=(320, 320),
        stride=8)
