...
class ResUNet(nn.Module):
    ...
    def __init__(self, name, coarse_dim, fine_dim):
        ...
        self.scaling_step_coarse = 8
        self.scaling_step_fine = 2
        ...
        self.iconv3 = ConvBN(1024, 512, 3, 1)
        self.upconv2 = UpConv(512, 256, 3, 2)
        self.iconv2 = ConvBN(512, 256, 3, 1)
        # coarse conv
        self.conv_coarse = ConvBN(1024, coarse_dim, 1, 1)
        #self.conv_coarse = Conv2d(1024, coarse_dim, 1, 1)
        # fine conv
        self.conv_fine = ConvBN(256, fine_dim, 1, 1)

        self.conv0 = Conv2d(512, 512, kernel_size=1, stride=1)
    ...
    def forward(self, inputs):
        x = self.firstrelu(self.firstbn(self.firstconv(inputs)))  # 64-d [n, 512, 120, 160]

        x1 = self.layer1(x)  # 64-d [n, 64, 240, 320]
        x2 = self.layer2(x1)  # 128-d [n, 128, 120, 160]
        x3 = self.layer3(x2)  # 256-d [n, 256, 60, 80]

        coarse_feature = self.conv_coarse(x3)  # coarse_dim-d [n, 1024, 60, 80]

        x = self.upconv3(x3)  # 512-d [n, 512, 120, 160]
        x = self.skipconnect(x2, x)  # 640-d [n, 640, 120, 160]
        x = self.iconv3(x)  # 512-d [n, 512, 120, 160]
        x = self.upconv2(x)  # 256-d [n, 256, 240, 320]
        x = self.skipconnect(x1, x)
        x = self.iconv2(x)  # 256-d [n, 256, 240, 320]

        fine_feature = self.conv_fine(x)  # fine_dim-d [n, 1, 240, 320]
