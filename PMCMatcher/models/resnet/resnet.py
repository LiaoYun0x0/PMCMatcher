...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 norm_layer=None,include_top=True):
        ...
        self.upconv2 = UpConv(256, 256, 3, 2)
        self.iconv3 = nn.Conv2d(512, 256,1,1,0,bias=False)

        self.conv_fine = nn.Conv2d(320, 256, 1, 1)
    ...
    def extract_endpoints(self,x):
        conv_fine = self.conv1(x) # 2
        x = self.bn1(conv_fine)
        x = self.relu(x)
        x = self.maxpool(x) # 4
        ...
        upx2 = self.upconv3(x2)  
        upx2 = self.skipconnect(upx2, x1)  
        upx2 = self.iconv3(upx2)  # [n, 256, 80, 80]
        upx = self.upconv2(upx2)  # [n, 256, 160, 160]
        upx0 = self.skipconnect(upx, conv_fine) # [n, 320, 160, 160]

        x5 = self.conv_fine(upx0)  # [n, 256, 160, 160]
